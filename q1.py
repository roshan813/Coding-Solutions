...
class Solution:
    ...
    def topological_sort(self):
        order = []
        q = collections.deque([task for task in self.tasks if self.in_deg[task] == 0])
...
s = Solution(tasks, durations, dependencies)
earliest_finish_time, latest_finish_time = s.calculate_times()
